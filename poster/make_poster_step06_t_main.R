...
txt_t_main_cm1 <- qplot(1:9, 1:9, geom = "blank") +
  ...
  annotate("text", x = 5, y = 2, label = "Period: January 2014", size = 12, colour = col_text2) +
  theme_blank
...
txt_t_main_rcm2 <- qplot(1:9, 1:9, geom = "blank") +
  ...
  annotate("text", x = 5, y = 2, label = "Zooming in/out with rMaps", size = 12, colour = col_text2) +
  theme_blank
